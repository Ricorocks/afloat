            ...
            class="mx-auto grid max-w-2xl grid-cols-1 items-start gap-x-8 gap-y-16 sm:gap-y-24 lg:mx-0 lg:max-w-none lg:grid-cols-2">
                <div class="lg:pr-4">
                    <div class="relative overflow-hidden rounded-3xl bg-gray-900 px-6 pb-9 pt-64 shadow-2xl sm:px-12 lg:max-w-lg lg:px-8 lg:pb-8 xl:px-10 xl:pb-10">
                      <img class="absolute inset-0 h-full w-full object-cover brightness-125" src="{{ $content->image->url ?? 'https://images.unsplash.com/photo-1630569267625-157f8f9d1a7e?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=2669&q=80' }}" alt="">
                      ...
                      <figure class="relative isolate">
                        ...
                        <img src="{{ asset('assets/logos/afloat/afloat-white.svg') }}" alt="" class="h-10 w-auto">
                        <blockquote class="mt-6 text-xl font-semibold leading-8 text-white">
                          <p>{{ $content->image_content }}</p>
                        </blockquote>
                      </figure>
                    </div>
                  </div>
                <div class="flex flex-col gap-3">
                    <p class="text-base font-semibold leading-7 text-brandblue">{{ $content->pre_title }}</p>
                    <h1 class="mt-2 text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl">{{ $content->title }}</h1>
                    <div class="max-w-xl ">
                        <div class="prose">{!! $content->content !!}</div>
                    </div>
                    <a href="#" class="text-base font-semibold leading-7 
                        text-brandblue hover:text-branddarkblue hover:underline">Learn more about our
